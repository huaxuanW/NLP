...
if __name__ == "__main__":
    ...
    batch_size, seq_len, embedding_dim = 10, 50, 512
    q = torch.rand((batch_size, seq_len, embedding_dim))
    k = torch.rand((batch_size, seq_len - 1, embedding_dim))
    v = torch.rand((batch_size, seq_len - 1, embedding_dim))

    att = MultiHeadAttention(embedding_dim, 8)
    res = att(q, k, v)
    print(res.shape)
